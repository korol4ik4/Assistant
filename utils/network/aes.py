...
def shot_encrypt(input_bytes, key):
    """Function encrypts the input_bytes according to AES(128) algorithm using the key
    Args:
       input_bytes -- bytes. Length of input_bytes is constantly 16
       key -- bytes. Do not forget it! The same bytes is used in decryption   
    Returns:
        List of int
    """
    ...
    output = data_to_string(output)
    return output.encode()


def shot_decrypt(cipher, key):
    ...
    cipher = [ord(ch) for ch in cipher.decode()]
    ...
    #output to bytes string
    output = data_to_string(output)
    # ''.join([chr(out) for out in output])
    return output.encode()
...
def encrypt(input_bytes, key):
	input_blocks = block_list(input_bytes)
	out_list= []
	for block in input_blocks:
		out_list.append(shot_encrypt(block,key))
	return out_list

def decrypt(cipher_blocks, key):
	#cipher_blocks = block_list(cipher)
	out_list= []
	for block in cipher_blocks:
		out_list.append(shot_decrypt(block,key))
	output = b''.join(out_list)
	return output
	


def block_list(message, size=16):
	msg_len=len(message)
	nums = msg_len//size+1
	#+1, что б гарантировать "\0" в конце
	
	start=0
	end =size
	blocks=[]
	for i in range(nums):
		#если сообщение короче блока
		if end > msg_len:
			dl = end - msg_len
			block = (message[start:msg_len].decode()+'\0'*dl).encode()
		else:
